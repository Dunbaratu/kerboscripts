...
lock throttle to 1.
lock steering to HEADING(comp, 85 - 43*(sqrt(ship:velocity:surface:mag/1000))).
...
stage.
wait 1.
local FUR1 is stage:Furfuryl.
wait 1.
local FUR2 is stage:Furfuryl.
local FURrate is FUR1 - FUR2.
print "Burning Furfuryl at " + FURrate + " per second.".
wait until stage:Furfuryl < 2*FURRate.
...
print "Activate Science!".
toggle AG1.
print "Waiting for APO".
wait until eta:apoapsis < 5.
print "Pre-staging chutes".
stage.
unlock steering.
wait 2.
print "Unlocking steering. Pre-staging for chutes.".
print "Tumbling helplessly. Script over.".
